...
class Snake extends FlexiblePictureExplorer implements ImageObserver, ActionListener, MouseListener {
	...
	public Snake() {
		...
		// make a global variable, "direction", that changes according to the direction
		// the snake is going
		// just change the variable direction whenever needed, the function called by
		// the timer will automatically use the variable and update whenever it is
		// updated
		// updateImage();
	}
	...
	private ArrayList<snakePiece> snakePieces = new ArrayList<snakePiece>();
	private int bodyPieces = 1;
	...
	public void runTimer() {
		...
		try {
			...
			snakePiece initialPiece = new snakePiece(xCoordinateOfSnakePiece, yCoordinateOfSnakePiece);
			snakePieces.add(initialPiece);
			appleCreater();
			playing = 1;
			while (playing != 0) {
				...
				Thread.sleep(10);
				graphics.drawImage(playScreen.getBufferedImage(), 0, 0, this);
				graphics.drawImage(snakePiece.getBufferedImage(), xCoordinateOfSnakePiece, yCoordinateOfSnakePiece,
						this);
				int currentXPos = xCoordinateOfSnakePiece;
				int currentYPos = yCoordinateOfSnakePiece;
				...
				for (int i = 1; i < snakePieces.size(); i++) {
					...
					// currentXPos = snakePieces.get(i - 1).getX();
					// currentYPos = snakePieces.get(i - 1).getY();
					graphics.drawImage(snakePiece.getBufferedImage(), snakePieces.get(i - 1).getX(),
							snakePieces.get(i - 1).getY(), this);

				}
				if (playing == 1) {
					appleCreater();
				} else if (checkBlockCollisions(randomX, randomY, 30)) {
					appleCreater();
					if (direction.equals("L")) {
						snakePieces.add(new snakePiece(xCoordinateOfSnakePiece - speed, yCoordinateOfSnakePiece));
					} else if (direction.equals("R")) {
						snakePieces.add(new snakePiece(xCoordinateOfSnakePiece + speed, yCoordinateOfSnakePiece));
					} else if (direction.equals("U")) {
						snakePieces.add(new snakePiece(xCoordinateOfSnakePiece, yCoordinateOfSnakePiece - speed));
					} else if (direction.equals("D")) {
						snakePieces.add(new snakePiece(xCoordinateOfSnakePiece, yCoordinateOfSnakePiece + speed));
					}
					score++;
	...
	public boolean checkBlockCollisions(int x, int y, int widthAndHeight) {
		int leftXOfApple = x;
		...
		if ((rightXOfSnake > leftXOfApple) && ((leftXOfSnake < rightXOfApple)) && (bottomYOfSnake > topYOfApple)
				&& (topYOfSnake < bottomYOfApple)) {
	...
	public void appleCreater() {
		...
		while (checkBlockCollisions(randomX, randomY, 30) || checkCollisions(randomX, randomY, 30)) {
			randomX = rand.nextInt(659) + 181;
			randomY = rand.nextInt(361) + 34;
